...
public class DbUse {
    ...
    public int CountRecords() { // select all the field of the foo itmes table
        int count = 0;
        try {
    ...
    public int FoodItemByCatCount(String cat_id) { // to count the numbero f rows on the basis of some query
        int count = 0;
        try {
    ...
    public int CountByOrderId(String order_no) { // to coutn the number of rows in the talbe
        int count = 0;
        try {
    ...
    public int CountCategory() { // function to display all the categoires
        int count = 0;
        try {
            pstm = con.prepareStatement("SELECT * FROM categories");
            result = pstm.executeQuery();
    ...
    public ResultSet FoodItemDisplay() {
        try {
            pstm = con.prepareStatement("SELECT * FROM fooditems ORDER BY cat_id"); // ordering the output by the cat_id
            result = pstm.executeQuery();
        } catch (Exception ex) {
            ex.getMessage();
        }
    ...
    public int CountFoodItemDisplayByCat(String cat_id) {
        int count = 0;
        try {
            pstm = con.prepareStatement("SELECT * FROM fooditems WHERE fooditems.cat_id = ?");
            pstm.setString(1, cat_id);
